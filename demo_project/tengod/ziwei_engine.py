...
class ZiweiEngine:
    ...
    # ── 农历转换（简化版，基于1900-2100年数据） ──
    # 农历数据：每年16位编码（前12位为每月大小月1大0小，后4位为闰月，0表示无闰月）
    _LUNAR_INFO = {
        2000: 0x04bd8, 2001: 0x04ae0, 2002: 0x0a570, 2003: 0x054d5, 2004: 0x0d260,
        2005: 0x0d950, 2006: 0x16554, 2007: 0x056a0, 2008: 0x09ad0, 2009: 0x055d2,
        2010: 0x04ae0, 2011: 0x0a5b6, 2012: 0x0a4d0, 2013: 0x0d250, 2014: 0x1d255,
        2015: 0x0b540, 2016: 0x0d6a0, 2017: 0x0ada2, 2018: 0x095b0, 2019: 0x14977,
        2020: 0x04970, 2021: 0x0a4b0, 2022: 0x0b4b5, 2023: 0x06a50, 2024: 0x06d40,
        2025: 0x1ab54, 2026: 0x02b60, 2027: 0x09570, 2028: 0x052f2, 2029: 0x04970,
        2030: 0x06566, 2031: 0x0d4a0, 2032: 0x0ea50, 2033: 0x16a95, 2034: 0x05ad0,
        2035: 0x02b60, 2036: 0x186e3, 2037: 0x092e0, 2038: 0x1c8d7, 2039: 0x0c950,
        2040: 0x0d4a0, 2041: 0x1d8a6, 2042: 0x0b550, 2043: 0x056a0, 2044: 0x1a5b4,
        2045: 0x025d0, 2046: 0x092d0, 2047: 0x0d2b2, 2048: 0x0a950, 2049: 0x0b557,
        2050: 0x06ca0,
    }

    # 农历年对应的正月初一公历日期偏移（相对1月1日）
    _LUNAR_NEW_YEAR = {
        2000: 5, 2001: 24, 2002: 12, 2003: 1, 2004: 22, 2005: 9, 2006: 29,
        2007: 18, 2008: 7, 2009: 26, 2010: 14, 2011: 3, 2012: 23, 2013: 10,
        2014: 31, 2015: 19, 2016: 8, 2017: 28, 2018: 16, 2019: 5, 2020: 25,
        2021: 12, 2022: 1, 2023: 22, 2024: 10, 2025: 29, 2026: 17, 2027: 6,
        2028: 26, 2029: 13, 2030: 3, 2031: 23, 2032: 11, 2033: 31, 2034: 19,
        2035: 8, 2036: 28, 2037: 15, 2038: 4, 2039: 24, 2040: 12, 2041: 1,
        2042: 21, 2043: 10, 2044: 30, 2045: 17, 2046: 6, 2047: 26, 2048: 14,
        2049: 2, 2050: 23,
    }
    ...
    def _lunar_month_days(cls, year: int, month: int) -> int:
        """获取农历月的天数"""
        info = cls._LUNAR_INFO.get(year, 0)
        if info & (1 << (15 - (month - 1))):
            return 30
        return 29
    ...
    def solar_to_lunar(cls, year: int, month: int, day: int) -> Tuple[int, int, int, bool]:
        ...
        """
        # 计算距离正月初一的天数
        new_year_offset = cls._LUNAR_NEW_YEAR.get(year, 1)
        doy = cls._day_of_year(year, month, day)
        
        if doy >= new_year_offset:
            # 在当年农历年内
            offset = doy - new_year_offset
            lunar_year = year
        else:
            # 在上一年农历年内
            prev_year = year - 1
        ...
        while offset >= 0:
            if lunar_month == leap_month + 1 and not is_leap and leap_month > 0:
                is_leap = True
                ...
                if is_leap and lunar_month == leap_month:
                    ...
                    lunar_month = lunar_month + 1 if not is_leap else lunar_month
            
            month_days = cls._lunar_month_days(lunar_year, lunar_month)
    ...
    def _get_month_gan(cls, year_gan: str, month_zhi_idx: int) -> str:
        ...
        start = (base * 2) % 10  # 甲己→丙寅, 乙庚→戊寅, 丙辛→庚寅, 丁壬→壬寅, 戊癸→甲寅
