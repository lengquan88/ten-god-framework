...
class BaziReportGenerator:
    ...
    def markdown_report(self) -> str:
        ...
        lines.append(f"## 三、五行分析")
        lines.append(f"")
        lines.append(f"| 五行 | 数量 | 评分 | 状态 |")
        lines.append(f"|------|------|------|------|")
        for wx in ["木", "火", "土", "金", "水"]:
            count = a['wuxing'].get(wx, 0)
            score = a['wuxing_score'].get(wx, "-")
