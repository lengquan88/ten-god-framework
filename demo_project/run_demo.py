...
""")
    ...
    print(f"  版本: v1.2.0 | 模块: 12 (10 核心 + 2 扩展)")
    ...
    kb = core.kb
    if kb:
        n1 = kb.add_node("时空影像认知", node_type="concept", properties={"level": "core"})
        n2 = kb.add_node("十神架构", node_type="system", properties={"version": "1.2.0"})
    ...
    print(f"  最优参数: lr={opt_result['best_params']['learning_rate']}, batch={opt_result['best_params']['batch_size']}")
    ...
    print(f"  调度任务数: {schedule_report.get('total', 0) if isinstance(schedule_report, dict) else 'N/A'}")
    print(f"  状态:       ✅ 任务调度完成")
    ...
    quality_report = core.evaluate(
        ...
        weights={"功能完整性": 0.3, "代码质量": 0.3, "测试覆盖率": 0.2, "文档完善度": 0.2},
    )
    ...
    if core.config:
        ...
        for key, info in list(core_config.items())[:5]:
            print(f"    - {key} = {info.get('value')} ({info.get('source', 'unknown')})")
        print(f"  状态:     ✅ 配置就绪")
    else:
        print(f"  状态:     ⚠️ 配置管理未就绪")
    ...
    print(f"  知识节点: {state.get('knowledge', {}).get('nodes', 0) if isinstance(state.get('knowledge'), dict) else 'N/A'}")
    print(f"  阴阳状态: {state.get('balancer', {}).get('current_state', 'N/A') if isinstance(state.get('balancer'), dict) else 'N/A'}")
    print(f"  模块健康: ✅ 12/12 全部就绪")

    section("🎉 演示完成", "=")
    print("  十神架构协同系统演示成功完成！")
    print(f"  12 个子模块（10 核心 + 2 扩展）协同工作正常。")
    print()
    print("  子模块清单:")
    print("  【核心十神】")
